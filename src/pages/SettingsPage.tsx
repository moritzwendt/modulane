          ...
          {tab === "Workspace" && (
            <form onSubmit={saveWorkspace}>
              <SettingsSection title="Organisationsprofil" description="Diese Angaben sehen alle Personen in deiner Organisation.">
                ...
                <div className="field-group"><label htmlFor="workspace-visibility">Zugriff</label><AppSelect id="workspace-visibility" value={workspaceInput.visibility} disabled={!permissions.canManageSecurity} onValueChange={(visibility) => setWorkspaceInput({ ...workspaceInput, visibility: visibility as WorkspaceVisibility })} options={["Nur auf Einladung", "Offen für die Organisation"].map((visibility) => ({ value: visibility, label: visibility }))} />{!permissions.canManageSecurity && <p className="field-helper">Nur Eigentümer können den Organisationszugriff ändern.</p>}</div>
              </SettingsSection>
              <SettingsSection title="Standardwerte" description="Neue Projekte starten mit diesen Einstellungen.">
                <div className="field-group"><label htmlFor="default-project-status">Standardstatus für Projekte</label><AppSelect id="default-project-status" value={workspaceInput.defaultProjectStatus} onValueChange={(defaultProjectStatus) => setWorkspaceInput({ ...workspaceInput, defaultProjectStatus: defaultProjectStatus as ProjectStatus })} options={projectStatuses.map((status) => ({ value: status, label: status }))} /></div>
