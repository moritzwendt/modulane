...
export function ProjectSettingsPage() {
  ...
  const { projects, users, currentUserId, settings, updateProject } = useWorkspace()
  ...
  if (!organizationPermissions(currentUser.role, settings).canManageOrganization) return <Navigate to={`/projects/${project.id}`} replace />
    ...
    <div className="page project-settings-page">
      ...
      <form className="project-settings-form" onSubmit={submit}>
        <SettingsBlock icon={<Archive size={17} />} title="Projekt" description="Grundlegende Angaben und aktueller Projektstatus.">
          ...
          <div className="form-grid"><div className="field-group"><label htmlFor="settings-project-type">Typ</label><AppSelect id="settings-project-type" value={input.type} onValueChange={(type) => setInput({ ...input, type: type as ProjectType })} options={projectTypes.map((type) => ({ value: type, label: type }))} /></div><div className="field-group"><label htmlFor="settings-project-status">Status</label><AppSelect id="settings-project-status" value={input.status} onValueChange={(status) => setInput({ ...input, status: status as ProjectStatus })} options={projectStatuses.map((status) => ({ value: status, label: status }))} /></div></div>
        ...
        <SettingsBlock icon={<LockSimple size={17} />} title="Zugriff" description="Lege fest, wer dieses Projekt finden und öffnen kann.">
          <div className="access-options"><label className={input.visibility === "Workspace" ? "selected" : ""}><input type="radio" name="visibility" checked={input.visibility === "Workspace"} onChange={() => setInput({ ...input, visibility: "Workspace" as ProjectVisibility })} /><span><strong>Organisation</strong><small>Alle Mitglieder können das Projekt sehen.</small></span><Check size={16} /></label><label className={input.visibility === "Privat" ? "selected" : ""}><input type="radio" name="visibility" checked={input.visibility === "Privat"} onChange={() => setInput({ ...input, visibility: "Privat" as ProjectVisibility })} /><span><strong>Privat</strong><small>Nur ausgewählte Mitglieder haben Zugriff.</small></span><Check size={16} /></label></div>
        </SettingsBlock>
        <SettingsBlock icon={<Users size={17} />} title="Projektteam" description="Diese Personen können Aufgaben sehen und an Komponenten arbeiten.">
